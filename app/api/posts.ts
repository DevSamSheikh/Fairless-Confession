...
export interface CreatePostBody {
  title?: string | null; // Allow null for empty titles
  content: string;
  category: string;
  visibility?: string;
...
export async function createPost(
  ...
  try {
    const res = await fetch(`${baseUrl}/api/post`, {
      ...
      body: JSON.stringify({
        title: body.title?.trim() || null, // Send null for empty/undefined titles
    ...
    if (res.status === 204) {
      throw new Error(
        "Server returned no content. Please check your connection and try again.",
      );
    }
    ...
    if (!res.ok) {
      ...
      if (res.status === 400) {
        ...
        if (
          data?.sanitizedTitle !== undefined &&
          data?.sanitizedContent !== undefined
        ) {
          throw new ContentBlockedError(
            data.error || "Content not allowed",
            data.sanitizedTitle,
